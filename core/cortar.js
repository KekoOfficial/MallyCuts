...
async function extraerYEditarSegmento(ruta, titulo) {
  return new Promise((res, rej) => {
    ...
    .videoFilters(`setpts=1/${config.VELOCIDAD}*PTS,scale=-2:720,format=yuv420p`)
    .audioFilters(audio(config.VELOCIDAD))
    .outputOptions(['-f segment','-segment_time 120','-reset_timestamps 1'])
    .outputFormat('mp4').output(out);
