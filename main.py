...
# Optimización de red para Termux
...
def motor_mally_pro(p_vid, p_port, nombre, desc):
    ...
    # 2. Edición: Marca de Agua (Video Maestro)
    p_vid_editado = p_vid.replace(".mp4", "_edit.mp4")
    # Usamos preset ultrafast para que no tarde tanto en el celular
    cmd_edit = [
        'ffmpeg', '-y', '-i', p_vid,
        '-vf', f"drawtext=text='{config.WATERMARK_TEXT}':x=w-tw-20:y=h-th-20:fontcolor={config.WATERMARK_COLOR}:fontsize=24",
        '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '24', '-c:a', 'copy', p_vid_editado
    ]
    ...
    for i in range(total):
        ...
        # Corte rápido desde el maestro editado
        subprocess.run(['ffmpeg', '-y', '-ss', str(i*config.CLIP_DURATION), '-t', str(config.CLIP_DURATION), '-i', p_vid_editado, '-c', 'copy', '-avoid_negative_ts', '1', salida], check=True)
