...
def motor_mallycuts_express(video_path, portada_path, nombre, descripcion):
    try:
        ...
        for i in range(total):
            ...
            (ffmpeg.output(final_v, v_in.audio, out_p, vcodec='libx264', preset='ultrafast')
             .overwrite_output().run(quiet=True))

            # 4. Envío Blindado (con Timeout extendido)
            with open(out_p, 'rb') as v:
                cap = (f"🎬 {nombre}\n"
                       f"💎 PARTE: {i+1} / {total}\n"
                       f"✅ Contenido Verificado\n"
                       f"🔗 @MallySeries #UmbraeStudio")
                bot.send_video(config.CHAT_ID, v, caption=cap, 
...
if __name__ == "__main__":
    print("🧠 Cerebro de Umbrae Studio sincronizado.")
